...
const ImageUploadField: React.FC<IImageUploadFieldProps> = ({
  ...
  const handleFileChange = (
    ...
    const totalFiles = [...currentValue, ...newFiles].slice(0, maxFiles)
        ...
        <div className='w-full'>
          ...
          >
            <input
              ...
              multiple
              className='hidden'
              onChange={(e) => handleFileChange(e.target.files, value, onChange)}
            />
              ...
              {previews.map((preview, index) => (
                <div key={index} className='relative group'>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      removeFile(index, value, onChange)
                    }}
                  >
                    ✕
                  </button>
          ...
          {value.length > 0 && (
            <p className='text-sm text-green-600 mt-3 flex items-center gap-1'>
              <span>✓</span> {value.length}/{maxFiles}枚選択中
            </p>
          )}
